...
void setDuty(const struct pwm_dt_spec pwm, uint16_t max_period)
{
	/*
...
*/
	...
	setDuty(pwm_led1, max_period);
	setDuty(pwm_led2, max_period);
	setDuty(pwm_led3, max_period);
	...
	period = max_period;
	while (1) {
		ret = pwm_set_dt(&pwm_led0, period, period / 20U);
		if (ret) {
			printk("Error %d: failed to set pulse width 0\n", ret);
			return;
		}
		ret = pwm_set_dt(&pwm_led1, period, period / 20U);
		if (ret) {
			printk("Error %d: failed to set pulse width 1\n", ret);
			return;
		}
		ret = pwm_set_dt(&pwm_led2, period, period / 10U);
		if (ret) {
			printk("Error %d: failed to set pulse width 2\n", ret);
			return;
		}
		ret = pwm_set_dt(&pwm_led3, period, period / 10U);
		if (ret) {
			printk("Error %d: failed to set pulse width 3\n", ret);
			return;
		}
		/*
